...
public class DateReadingsUtil {
    ...
    public static final String FORMAT_TO_DISPLAY="dd MMM - HH:mm:ss";
    ...
    public static List<TempReading> cloneLists(List<TempReading> list) {
        List<TempReading> copy = new ArrayList<TempReading>(list.size());
        for (TempReading tr : list) {
            try {
                copy.add((TempReading) tr.clone());
            } catch (CloneNotSupportedException e) {
                e.printStackTrace();
            }
        }
    ...
    //method modified preserve the original rawResult array
    public static List<TempReading> roundToXMinutes(List<TempReading> result, int minutes, String format) {
        List<TempReading> resultCopy = new ArrayList<TempReading>(result.size());

        for (TempReading reading : result) {

            TempReading readingCopy = null;
            try {
                readingCopy = (TempReading) reading.clone();
            } catch (CloneNotSupportedException e) {
                e.printStackTrace();
            }

            Date date = readingCopy.getDate();
            ...
            readingCopy.setDate(calendar.getTime());

            resultCopy.add(readingCopy);
        }

        resultCopy = averageToUnits(resultCopy, format);
